...
class Application:
    ...
    def __init__(self):
        ...
        self.model = load_model('Proyecto final\cnn8grps_rad1_model.h5')
        ...
        for i in ascii_uppercase:
            self.ct[i] = 0
        print("Cargando modelo del disco")


        self.root = tk.Tk()
        self.root.title("Conversion de lenguaje de señas a texo/voz")
        ...
        self.T.config(text="Conversion de lenguaje de señas a texto/voz :)", font=("Courier", 30, "bold"))
        ...
        self.T1.config(text="Letra :", font=("Courier", 30, "bold"))
        ...
        self.T3.config(text="Frase :", font=("Courier", 30, "bold"))
        ...
        self.speak.config(text="Hablar", font=("Courier", 20), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1155, y=630)
        self.clear.config(text="Limpia", font=("Courier", 20), wraplength=100, command=self.clear_fun)
    ...
    def predict(self, test_image):
        ...
        if ch1 == 'Aplicar' or 'B' or 'C' or 'H' or 'F' or 'X':
        ...
        if ch1=="Aplicar" and self.prev_char!="Aplicar":
            if self.ten_prev_char[(self.count-2)%10]!="Aplicar":
                if self.ten_prev_char[(self.count-2)%10]=="Borrar":
                    self.str=self.str[0:-1]
                else:
                    if self.ten_prev_char[(self.count - 2) % 10] != "Borrar":
                        self.str = self.str + self.ten_prev_char[(self.count-2)%10]
            else:
                if self.ten_prev_char[(self.count - 0) % 10] != "Borrar":
